...
sinput string          _SSL_ = "─────── STOP LOSS ADAPTATIVO (ATR) ───────";
input  bool            InpUseATRSL    = true;          // Activar SL adaptativo ATR
input  int             InpATRPeriod   = 14;            // ATR Periodo
input  double          InpATRMultSL   = 2.5;           // SL = ATR x este factor (margen al mercado)
input  double          InpMaxLossUSD  = 0.0;           // Tope perdida USD por trade (0=off)

sinput string          _SHTF_ = "─────── FILTRO TENDENCIA TF SUPERIOR ───────";
input  bool            InpUseHTF      = true;          // Confirmar con TF superior
input  int             InpHTFEmaFast  = 50;            // EMA rapida HTF
input  int             InpHTFEmaSlow  = 200;           // EMA lenta HTF
...
//+------------------------------------------------------------------+
//| FILTRO TENDENCIA HTF: 1=alcista, -1=bajista, 0=indef            |
//+------------------------------------------------------------------+
int HTFTrend()
{
   if(!InpUseHTF) return 0; // 0 = sin filtro (permite ambos)
   double f[], s[];
...
double GetATR()
{
   ...
   if(CopyBuffer(hATR, 0, 0, 2, a) < 2) return 0;
   return a[1];
}
...
bool InitIndicators()
{
   ...
   // ATR para SL adaptativo
   hATR     = iATR(_Symbol, botTF, InpATRPeriod);

   // EMAs de tendencia en TF superior (confirmacion)
   ENUM_TIMEFRAMES htf = HigherTF(botTF);
   hHTFFast = iMA(_Symbol, htf, InpHTFEmaFast, 0, MODE_EMA, PRICE_CLOSE);
   hHTFSlow = iMA(_Symbol, htf, InpHTFEmaSlow, 0, MODE_EMA, PRICE_CLOSE);
...
bool OpenTrade(bool buy)
{
   ...
   // --- Calcular SL adaptativo por ATR ANTES de abrir ---
   double slPrice = 0;
   double atr = GetATR();
   double refPrice = buy ? SymbolInfoDouble(_Symbol, SYMBOL_ASK)
                         : SymbolInfoDouble(_Symbol, SYMBOL_BID);
   if(InpUseATRSL && atr > 0)
   {
      double slDist = atr * InpATRMultSL;
      slPrice = buy ? refPrice - slDist : refPrice + slDist;
      slPrice = NormalizeDouble(slPrice, _Digits);
   }

   bool   ok  = buy ? Trade.Buy(lot, _Symbol, 0, slPrice, 0, "SMP_BUY")
                    : Trade.Sell(lot, _Symbol, 0, slPrice, 0, "SMP_SELL");
   if(!ok)
   ...
   Print("Trade opened: ", buy?"BUY":"SELL", " Lot=", lot,
         " Entry=", entryPrice, " SL=", slPrice, " ATR=", DoubleToString(atr,_Digits));
   return true;
}
...
void ManageGridTP(int bar)
{
   ...
   // --- Tope de perdida USD (red de seguridad) ---
   if(InpMaxLossUSD > 0 && profit <= -InpMaxLossUSD)
   {
      Print("Max USD loss hit (", DoubleToString(profit,2), "). Closing.");
      ...
      switch(botState)
      {
         ...
         case STATE_MA_CROSS:
         {
            ...
            if(bar >= maCrossBar + 2)
            {
               ...
               // Confirmacion tendencia TF superior
               int htf = HTFTrend();
               if(htf != 0 && ((maDir && htf != 1) || (!maDir && htf != -1)))
               {
                  Print("HTF trend blocked entry (htf=", htf, ", dir=", maDir, "). Reset.");
                  botState = STATE_IDLE;
                  break;
               }
