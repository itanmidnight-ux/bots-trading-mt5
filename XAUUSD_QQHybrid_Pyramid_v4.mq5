//+------------------------------------------------------------------+
//|  XAUUSD QQ HYBRID PYRAMID v4.0 – ENHANCED EDITION               |
//|  + Grid Lots | NY Session | ADX | Adaptive ATR | EMA Retest      |
//|  Bugs corregidos: FilterQQ1, breakoutTime, PartialClose vol       |
//+------------------------------------------------------------------+
#property copyright "QQ Hybrid Pyramid v4.0"
...
input double InpPyramidTrigger1 = 1.0;
input double InpPyramidTrigger2 = 2.0;
input double InpPyramidLotMult  = 0.75;
...
void ManagePyramid()
{
   ...
   double trigPts = (g_pyramidLevel == 0)
   ...
   double moveFav = masterIsBuy ? (curP - masterPrice) : (masterPrice - curP);
   if(moveFav < trigPts) return;
   ...
   if(masterIsBuy)
   {
      ...
      { g_pyramidLevel++; g_tradesToday++; Print("📈 PIRÁMIDE L", g_pyramidLevel, " @ ", ask, " Lot:", pyrLot); }
   }
   else
   {
      double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      ...
      { g_pyramidLevel++; g_tradesToday++; Print("📉 PIRÁMIDE L", g_pyramidLevel, " @ ", bid, " Lot:", pyrLot); }
   }
}
...
void ManageOpenTrades()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      { trade.PositionClose(tk); Print("⏱️ Cierre tiempo:", barsOpen, "barras"); continue; }
...
void ApplyTrailingATR(ulong ticket, bool isBuy, double atr)
{
   ...
   double trailD  = (atr > 0) ? atr * InpTrailingATRMult : 10 * _Point * 10;
   if(isBuy)
   {
      if(curP <= openP) return;
...
void DrawPanel()
{
   ...
   txt += StringFormat("Trades: %d/%d | Win: %d | Loss: %d\n", g_tradesToday, InpMaxTradesDay, g_winsToday, g_lossesToday);
