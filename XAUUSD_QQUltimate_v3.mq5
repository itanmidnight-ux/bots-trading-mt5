...
bool        g_weeklyBiasUp   = false;
bool        g_weeklyBiasDn   = false;
...
struct StratState
{
   ...
   int    gridLayers;          // capas grid actuales (solo strats grid)
   double avgEntry;
   bool   usesGrid;
   double lastSL, lastTP;
};
...
int OnInit()
{
   g_isTesting = (bool)MQL5InfoInteger(MQL5_TESTER);
   ...
   hEMA10_W1 = iMA(_Symbol, PERIOD_W1, 10, 0, MODE_EMA, PRICE_CLOSE);
   if(hADX_H1==INVALID_HANDLE || hADX_M15==INVALID_HANDLE || hEMA10_W1==INVALID_HANDLE)
   {
      Alert("QQ v10: Handle ADX/Weekly inválido");
      return INIT_FAILED;
   }

   // Init strategy states
...
void OnDeinit(const int reason)
{
   ...
   if(hEMA10_W1!= INVALID_HANDLE) IndicatorRelease(hEMA10_W1);
   PanelClear();
   Comment("");
}
...
void OnTick()
{
   ...
   bool canTrade = !g_dayInvalid && g_recovLevel < 3 && !IsRecoveryPaused() && !IsNewsTime();
                        ...
                        const MqlTradeResult&       res)
{
   ...
   if(entry == DEAL_ENTRY_OUT || entry == DEAL_ENTRY_OUT_BY)
   {
      ...
      if(profit > 0) { g_winsToday++; g_consecLosses = 0; }
      else if(profit < 0) { g_lossesToday++; g_consecLosses++; }
   }
}
...
void UpdateMTFData()
{
   ...
   for(int i=0; i<NUM_TFS; i++)
   {
      ENUM_TIMEFRAMES tf = gTFs[i];
      g_tf[i].valid = false;
      if(CopyBuffer(hEMA9[i],  0,0,1,e9)  <= 0) continue;
      if(CopyBuffer(hEMA21[i], 0,0,1,e21) <= 0) continue;
      if(CopyBuffer(hEMA50[i], 0,0,1,e50) <= 0) continue;
      if(CopyBuffer(hEMA200[i],0,0,1,e200)<= 0) continue;
      if(CopyBuffer(hRSI[i],   0,0,1,r)   <= 0) continue;
      if(CopyBuffer(hATR[i],   0,0,1,a)   <= 0) continue;
      if(CopyBuffer(hMFI[i],   0,0,1,mf)  <= 0) continue;
      if(CopyBuffer(hBB[i],    1,0,1,bU)  <= 0) continue;
      if(CopyBuffer(hBB[i],    0,0,1,bM)  <= 0) continue;
      if(CopyBuffer(hBB[i],    2,0,1,bD)  <= 0) continue;
      ...
      g_tf[i].barTime  = iTime(_Symbol,  tf, 0);
      g_tf[i].valid    = true;
   }
   ...
   double weBuf[];
   if(CopyBuffer(hEMA10_W1,0,0,1,weBuf) > 0) g_weeklyEMA = weBuf[0];
...
void UpdateWeeklyBias()
{
   ...
   g_weeklyBiasUp = (cur > g_weeklyEMA) && (g_weeklyEMA > 0);
   g_weeklyBiasDn = (cur < g_weeklyEMA) && (g_weeklyEMA > 0);
}
...
void ActivateStrategies()
{
   ...
   // S8 PDH/PDL Breakout — siempre activo en ventana
   g_str[7].activeRegime = (inPDHWind && r != REG_RANGING);
...
void RunActiveStrategies()
{
   ...
   // evalúan entrada en cada tick. Solo paramos si tocamos límite global
   // de concurrencia (g_maxConcurrent) — controlado dentro de OpenTrade().
   // Esto replica el comportamiento real de Quantum Queen: cuando el
   // detector identifica un régimen, todas sus estrategias se activan
   // y compiten por encontrar setups válidos.
   for(int s=0; s<NUM_STRATS; s++)
   {
      if(!g_str[s].activeRegime) continue;
      if(g_str[s].posCount > 0)  continue;
      if(CountAllOpen() >= g_maxConcurrent) break;   // límite alcanzado
      switch(s)
      {
...
bool TryS1_TrendFollow()
{
   ...
   if(buyPull && WeeklyBiasOk(true))   return OpenTrade(0, true,  PERIOD_M5, 1.5, 2.2);
   if(sellPull && WeeklyBiasOk(false)) return OpenTrade(0, false, PERIOD_M5, 1.5, 2.2);
   return false;
}
...
bool TryS2_PullbackEMA()
{
   ...
   if(touchUp && WeeklyBiasOk(true))   return OpenTrade(1, true,  PERIOD_H1, 1.5, 2.0);
   if(touchDn && WeeklyBiasOk(false))  return OpenTrade(1, false, PERIOD_H1, 1.5, 2.0);
   return false;
}
...
bool TryS3_LondonORB()
{
   ...
   if(breakUp) { g_orbFiredLong  = true; return OpenTrade(2, true,  PERIOD_M5, 1.3, 2.0); }
   if(breakDn) { g_orbFiredShort = true; return OpenTrade(2, false, PERIOD_M5, 1.3, 2.0); }
   return false;
}
...
bool TryS4_NYSession()
{
   ...
   if(dt.hour == 13 && dt.min < 30 && !g_nyBuilt)
   {
   ...
   if(breakUp) { g_nyFiredLong  = true; return OpenTrade(3, true,  PERIOD_M5, 1.4, 2.2); }
   if(breakDn) { g_nyFiredShort = true; return OpenTrade(3, false, PERIOD_M5, 1.4, 2.2); }
   return false;
}
...
bool TryS5_AsianRange()
{
   ...
   if(breakUp) { g_asFiredLong  = true; return OpenTrade(4, true,  PERIOD_M5, 1.3, 1.8); }
   if(breakDn) { g_asFiredShort = true; return OpenTrade(4, false, PERIOD_M5, 1.3, 1.8); }
   return false;
}
...
bool TryS6_MeanReversion()
{
   ...
   if(bounceUp)   return OpenTrade(5, true,  PERIOD_M5, 1.2, 1.5);
   if(bounceDn)   return OpenTrade(5, false, PERIOD_M5, 1.2, 1.5);
   return false;
}
...
bool TryS7_StructureBreak()
{
   ...
   if(brkUp && WeeklyBiasOk(true))   return OpenTrade(6, true,  PERIOD_H1, 1.6, 2.4);
   if(brkDn && WeeklyBiasOk(false))  return OpenTrade(6, false, PERIOD_H1, 1.6, 2.4);
   return false;
}
...
bool TryS8_PDHPDLBreak()
{
   ...
   if(brkUp)   return OpenTrade(7, true,  PERIOD_M15, 1.5, 1.8);
   if(brkDn)   return OpenTrade(7, false, PERIOD_M15, 1.5, 1.8);
   return false;
}
...
bool TryS9_MomentumM15()
{
   ...
   if(spikeUp)   return OpenTrade(8, true,  PERIOD_M15, 1.5, 2.5);
   if(spikeDn)   return OpenTrade(8, false, PERIOD_M15, 1.5, 2.5);
   return false;
}
...
bool TryS10_CompressionBreak()
{
   ...
   if(brkUp)   return OpenTrade(9, true,  PERIOD_M15, 1.3, 2.2);
   if(brkDn)   return OpenTrade(9, false, PERIOD_M15, 1.3, 2.2);
   return false;
}
...
bool TryS11_H1Continuation()
{
   ...
   if(contUp && WeeklyBiasOk(true))   return OpenTrade(10, true,  PERIOD_H1, 1.5, 2.2);
   if(contDn && WeeklyBiasOk(false))  return OpenTrade(10, false, PERIOD_H1, 1.5, 2.2);
   return false;
}
...
bool TryS12_VolumeImpulse()
{
   ...
   if(impUp)   return OpenTrade(11, true,  PERIOD_M6, 1.4, 2.0);
   if(impDn)   return OpenTrade(11, false, PERIOD_M6, 1.4, 2.0);
   return false;
}

//====================================================================
//  ============= LAYER 4 — GRID MANAGER =============
//  Solo para strats marcadas usesGrid=true (S1,S2,S7,S11)
//  Espaciado ATR × 1.0, max 3 capas, basket close al objetivo
//====================================================================
void ManageGrids()
{
   for(int s=0; s<NUM_STRATS; s++)
   {
      if(!g_str[s].usesGrid) continue;
      if(g_str[s].posCount == 0) continue;
      if(g_str[s].gridLayers >= MAX_GRID_LAYERS) continue;

      // Posición base
      ulong baseT = g_str[s].posTickets[0];
      if(!PositionSelectByTicket(baseT)) continue;
      bool isLong = (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY);
      double openPx = PositionGetDouble(POSITION_PRICE_OPEN);

      int iM5 = TFIdx(PERIOD_M5);
      if(iM5<0 || !g_tf[iM5].valid) continue;
      double atr = g_tf[iM5].atr;
      double spacing = atr * 1.0;

      double curPx = isLong ? SymbolInfoDouble(_Symbol, SYMBOL_BID)
                            : SymbolInfoDouble(_Symbol, SYMBOL_ASK);

      // ¿Última capa abrió en qué precio?
      double lastPx = openPx;
      for(int k=1; k<=g_str[s].gridLayers && k<MAX_GRID_LAYERS; k++)
      {
         if(PositionSelectByTicket(g_str[s].posTickets[k]))
            lastPx = PositionGetDouble(POSITION_PRICE_OPEN);
      }

      // Necesitamos: trade en profit + movimiento favorable + spacing alcanzado
      double moveFav = isLong ? (curPx - lastPx) : (lastPx - curPx);
      if(moveFav < spacing) continue;
      if(!IsBaseInProfit(s)) continue;

      // Verificar régimen siga siendo trend en dirección
      bool stillUp = g_str[s].sigCode > 0 && (g_h1TrendUp || g_d1TrendUp);
      bool stillDn = g_str[s].sigCode < 0 && (g_h1TrendDn || g_d1TrendDn);
      if(!stillUp && !stillDn) continue;

      // Añadir grid layer
      AddGridLayer(s, isLong);
   }
}

bool IsBaseInProfit(int sId)
{
   ulong t = g_str[sId].posTickets[0];
...
void AddGridLayer(int sId, bool isLong)
{
   ...
   int iM5 = TFIdx(PERIOD_M5);
   double atr = g_tf[iM5].atr;
   double sl  = isLong ? price - atr*1.5 : price + atr*1.5;
   double tp  = isLong ? price + atr*2.0 : price - atr*2.0;
   double lot = CalcLot(MathAbs(price - sl)) * 0.7;  // grid layers menores
...
void ManagePositions()
{
   ...
   // Basket close para grid strategies
   for(int s=0; s<NUM_STRATS; s++)
   {
      if(!g_str[s].usesGrid) continue;
      if(g_str[s].posCount < 2) continue;
      double basketPL = BasketPnL(s);
      double basketTgt = AccountInfoDouble(ACCOUNT_BALANCE) * 0.015 * g_str[s].gridLayers;
      if(basketPL >= basketTgt)
      {
         CloseStratPositions(s);
      }
   }
}
...
void DetectCapitalMode()
{
   ...
   double riskAdj = RiskLevelMult();   // de InpRiskLevel
   if(bal < 50.0)
   {
      g_capMode = CAP_MICRO;
...
{
   if(g_consecLosses >= 4)      { g_recovLevel = 3; g_lotMultRecov = 0.0; }
   else if(g_consecLosses >= 3) { g_recovLevel = 2; g_lotMultRecov = 0.50;
                                  if(g_recovPauseUntil == 0) g_recovPauseUntil = TimeCurrent() + 7200; }
...
{
   return (g_recovLevel == 2 && g_recovPauseUntil > 0 && TimeCurrent() < g_recovPauseUntil);
}
...
void DailyReset()
{
   ...
   for(int s=0; s<NUM_STRATS; s++)
   {
      g_str[s].winsToday = 0; g_str[s].lossesToday = 0; g_str[s].pnlToday = 0;
   }
...
{
   int n = 0;
   for(int s=0; s<NUM_STRATS; s++) n += g_str[s].posCount;
   return n;
}
...
{
   if(!g_weeklyBiasUp && !g_weeklyBiasDn) return true;   // neutro
   return isBuy ? !g_weeklyBiasDn : !g_weeklyBiasUp;
}
...
void DrawPanel()
{
   ...
   PanelLabel(PNL_PREFIX+"L19", 16, y, "Capital Mode: "+CapModeStr(), clrLightGray, InpPanelFontSize); y+=18;
   PanelLabel(PNL_PREFIX+"L20", 16, y, StringFormat("DD Lvl: %d | Recov: %d | Loss: %d/%d",
              g_ddLevel, g_recovLevel, g_lossesToday, g_consecLosses), clrLightGray, InpPanelFontSize); y+=18;
