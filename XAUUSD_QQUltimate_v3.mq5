//+------------------------------------------------------------------+
//|  XAUUSD QUANTUM QUEEN EVOLUTION v8.0                             |
//|                                                                  |
//|  Motores  : QQ Breakout | EMA Scalper | Retest                   |
//|             London ORB | Asian ORB | NY Session                  |
//|  Mejoras  : Sin pirámide | Enhancement 1R | Mín barras salida    |
//|             Veto D1 temporal | Control slippage ORB              |
//|             Filtro noticias | TP dinámico | Smart Recovery       |
//|  Gestión  : 10 Capas de Protección | Peak Lock | Smart Exit      |
//|                                                                  |
//|  Magic QQ = 5900 | ORB = 20250800 | ASIA = 20250900             |
//|         NY = 20251000                                            |
//+------------------------------------------------------------------+
#property copyright "QQ Evolution v8.0"
...
void DetectCapitalMode()
{
   ...
   // ── CAP_MICRO: $1 – $24.99 ───────────────────────────────────────
   // Prioridad: supervivencia del capital. Sin pirámide ni multi-trades.
   // Riesgo conservador, cierres rápidos, trailing ajustado.
   if(bal < 25.0)
   {
      g_capMode        = CAP_MICRO;
      g_riskPct        = 0.80;
      ...
      g_trailingMult   = 0.60;
      g_scalperRR      = 2.2;         g_maxTradesDay   = 4;
      g_maxBarsOpen    = 45;          g_maxNegBars     = 8;
      g_partialAt1R    = 0.50;        g_partialAt2R    = 0.30;
   }
   // ── CAP_SMALL: $25 – $59.99 ──────────────────────────────────────
...
{
   int base = (g_capMode <= CAP_SMALL) ? 5 : 6;
...
double CalcLot(double slPts, double riskMult)
{
   ...
   if(g_capMode == CAP_MICRO)
      lot = minLot;
   // Cuentas SMALL: máximo 2x lote mínimo por trade base (seguridad adicional)
              ...
              double &sl, double &tp, double rrMult=1.0)
{
   ...
   if(g_capMode == CAP_MICRO)
   {
      // Reducir el SL base para limitar la exposición con lote mínimo
      slD *= 0.75;
      // Aplicar cap duro: la pérdida con lote mínimo no supera InpMaxRiskPerTrade%
      if(g_microSLCapPts > 0 && slD > g_microSLCapPts)
         slD = g_microSLCapPts;
      // Recalcular TP proporcionalmente para mantener el RR configurado
      tpD = slD * (InpATR_TP_Mult / InpATR_SL_Mult) * rrMult;
   }
   else if(g_capMode == CAP_SMALL)
   {
      // Leve reducción del SL para cuentas pequeñas
      slD *= 0.90;
   }
...
void DrawPanel()
{
   ...
   string txt="";
   txt+="=== QQ ULTIMATE v8.0 EVOLUTION ===\n";
...
//+------------------------------------------------------------------+
//  FIN – QQ ULTIMATE v8.0 EVOLUTION
//+------------------------------------------------------------------+
